...
async function runSingleTest(test, currentNum, total) {
    let currentTest, result = {};
    test.fs = fs;
    currentTest = new Test(test);

    console.log(chalk.hex('#FFC300')(`Starting Test ---> ${currentTest.name}`));
    console.log(chalk.hex('#00FEC4').italic('results will be output to... ./results/' + currentTest.name + '\n'));
...
async function runAllTests(testsToRun) {
    ...
    fs.writeFile('summarized_geppetto_run.json', JSON.stringify(summaryData),(err) => {
        if (err) {
            console.log(err);
        } else {
            console.log(chalk.hex('#00FEC4').italic('Summary Data JSON saved on ./summarized_geppetto_run.json'));
    }});
    
    fs.writeFile('all_tests_data.json', JSON.stringify(geppettoSummary.getResultsData()),(err) => {
        if (err) {
            console.log(err);
        } else {
            console.log(chalk.hex('#00FEC4').italic('All Tests Data JSON saved on ./all_tests_data.json'));
        }
    });
